...
const Assessment = () => {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      // Send only the questions that were answered 'yes' along with their riskType and ID
      const triggeredQuestionsPayload = questions
        .filter(q => answers[q.id] === true)
        .map(q => ({
          id: q.id,
          riskType: q.riskType, // Send the risk type directly
          legalReference: q.legalReference, // Send legal reference
        }));

      // 2. Chama a Edge Function
      const { data: serverResult, error: funcError } = await supabase.functions.invoke('analyze-risk', {
        body: { triggeredQuestions: triggeredQuestionsPayload } // Updated payload key
      });
      ...
      // The serverResult now returns the triggered questions with their full data
      const triggeredQuestionsFromServer = serverResult?.triggeredQuestions || [];
      const triggeredIds = new Set(triggeredQuestionsFromServer.map((q: any) => q.id) || []);
